class CarouselSeries {
        ...
        try {
            ...
            this.seriesData = data
                .filter(item => item && typeof item === 'object' && 
                    item['Categoría'] && item['Categoría'].trim() !== '' && 
    ...
    async renderItems() {
        ...
        for (let i = this.index; i < end; i++) {
            ...
            if (item.series) metaInfo.push(`<span class="series-info">${item.series}</span>`);
            if (item.temporada) metaInfo.push(`<span class="season-info">T${item.temporada}</span>`);
